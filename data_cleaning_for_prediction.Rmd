...
**Getting rid of linear dependancy between columns**
...
One is "wins" column. It is the sum of all types of wins, so we have to remove it.
...
**Check the relation between response variable and X variables with boxplot**
...
for(i in c(1:20, 23:25)){
  boxplot(win_lose_data[,i], win_lose_data[, i+25], main=paste(strsplit(colnames(win_lose_data)[i], "_")[[1]][-1], sep=""), col=rainbow(2), outline=F)
  cat(paste(strsplit(colnames(win_lose_data)[i], "_")[[1]][-1], sep=""))
